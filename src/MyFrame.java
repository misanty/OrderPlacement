...
class MyFrame extends JFrame implements ActionListener {
	...
	private DataManOp dataManOp;
	private Customer lCus;
	...
	public MyFrame() {
		...
		Select.setLocation(100, 200);
		c.add(Select);

		Individual = new JRadioButton("Individual");
		Individual.setFont(new Font("Arial", Font.PLAIN, 15));
		Individual.setSelected(true);
		Individual.setSize(75, 20);
		Individual.setLocation(200, 200);
		c.add(Individual);

		Company = new JRadioButton("Company");
		Company.setFont(new Font("Arial", Font.PLAIN, 15));
		Company.setSelected(false);
		Company.setSize(80, 20);
		Company.setLocation(275, 200);
		c.add(Company);

		gengp = new ButtonGroup();
		gengp.add(Individual);
		gengp.add(Company);
		...
		if (e.getSource() == sub) {
			if (term.isSelected()) {
				...
				if (Individual.isSelected()) {
					data1 = "Select : Individual" + "\n";
				lCus = new Individual(tname.getText(), tmno.getText());
				lCus.changeFieldValue(FIELDS.ADDRESS, tadd.getText());
				dataManOp = lCus;
				...
				lCus = new Company(tname.getText(), tmno.getText(), 0);
				lCus.changeFieldValue(FIELDS.ADDRESS, tadd.getText());
				dataManOp = lCus;
				dataManOp.loadIt(lCus);
	...
	private void reg( String address, Customer c) throws IOException {
